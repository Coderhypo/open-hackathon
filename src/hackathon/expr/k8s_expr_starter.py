...
from hackathon.db.models import K8sEnvironment
from hackathon.db.models import Hackathon, VirtualEnvironment, Experiment
from hackathon.constants import (VirtualEnvProvider, VERemoteProvider, ExprEnvStatus, ExprStatus)
from hackathon.hackathon_response import internal_server_error
from hackathon.constants import K8sDeploymentStatus
...
class K8SExprStarter(ExprStarter):
    ...
    def __wait_for_k8s_ready(adapter, deployments, stateful_sets):
        ...
        for d_yaml in deployments:
            d = yaml.load(d_yaml)
            while adapter.get_deployment_status(d['metadata']['name']) != K8sDeploymentStatus.AVAILABLE:
                time.sleep(1)
    ...
    def __config_endpoint(self, expr, services):
        ...
        virtual_env.status = ExprEnvStatus.RUNNING
