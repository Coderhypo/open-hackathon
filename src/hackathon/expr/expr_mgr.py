...
from hackathon.constants import ExprStatus, VERemoteProvider, VirtualEnvProvider, ExprEnvStatus, ReservedUser, \
    HackathonNotifyEvent, HackathonNotifyCategory, CloudProvider, HackathonConfig
from hackathon.db.models import Experiment, User, Hackathon, UserHackathon, Template
...
class ExprManager(Component):
    ...
    def restart_stopped_expr(self, experiment_id):
        ...
        for ve in experiment.virtual_environments:
            if ve.provider == VirtualEnvProvider.DOCKER:
            ...
            elif ve.provider == VirtualEnvProvider.AZURE:
    ...
    def heart_beat(self, expr_id):
        expr = Experiment.objects(id=expr_id, status=ExprStatus.RUNNING).first()
        if expr is None:
    ...
    def check_expr_status(self, experiment):
        ...
        if all(x.status == ExprEnvStatus.RUNNING for x in virtual_environment_list) \
    ...
    def pre_allocate_expr(self, context):
        ...
        for template in hackathon_templates:
            try:
                template = template
                pre_num = int(hackathon.config.get(HackathonConfig.PRE_ALLOCATE_NUMBER, 1))
                query = Q(status=ExprStatus.STARTING) | Q(status=ExprStatus.RUNNING)
                ...
                # TODO Should support VirtualEnvProvider.K8S only in future after k8s Template is supported
                # if template.provider == VirtualEnvProvider.K8S:
                if curr_num < pre_num:
                    start_num = Experiment.objects(user=None, template=template, status=ExprStatus.STARTING).count()
                    allowed_currency = int(hackathon.config.get(HackathonConfig.PRE_ALLOCATE_CONCURRENT, 1))
    ...
    def __report_expr_status(self, expr, isToConfirmExprStarting=False):
        ...
        for ve in expr.virtual_environments:
            if ve.remote_provider == VERemoteProvider.Guacamole:
                try:
                    ...
                    ret["status"] = ExprStatus.STARTING
                    return ret
        ...
        public_urls = []
        if ve_provider == VirtualEnvProvider.DOCKER:
            for ve in expr.virtual_environments:
            ...
            raise NotImplementedError()
        elif ve_provider == VirtualEnvProvider.K8S:
    ...
    def __check_expr_real_status(self, experiment):
        ...
        for ve in experiment.virtual_environments:
            if ve.provider == VirtualEnvProvider.DOCKER:
                if not self.hosted_docker_proxy.is_container_running(ve.docker_container):
                    if ve.status == ExprEnvStatus.RUNNING:
                        ve.status = ExprEnvStatus.STOPPED
                else:
                    if ve.status == ExprEnvStatus.STOPPED:
                        ve.status = ExprEnvStatus.RUNNING
            elif ve.provider == VirtualEnvProvider.AZURE:
                raise NotImplementedError()
        if all(ve.status == ExprEnvStatus.STOPPED for ve in experiment.virtual_environments):
            experiment.status = ExprStatus.STOPPED
        if all(ve.status == ExprEnvStatus.RUNNING for ve in experiment.virtual_environments):
            experiment.status = ExprStatus.RUNNING
    ...
    def __recycle_expr(self, expr):
        ...
        if VirtualEnvProvider.DOCKER in providers:
