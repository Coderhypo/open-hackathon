...
from hackathon.constants import TemplateStatus, HackathonPlayerType, VirtualEnvProvider
from hackathon.db.pagination import Pagination
...
class Template(HDocumentBase):
    name = StringField(required=True, unique=True)
    provider = IntField()
    status = IntField(default=TemplateStatus.UNCHECKED)  # constants.TemplateStatus
    ...
    def unit_config(self):
        ...
        elif self.provider == VirtualEnvProvider.DOCKER:
    ...
    def _k8s_unit_config(self):
        ...
        return {
            TEMPLATE.VIRTUAL_ENVIRONMENT_PROVIDER: VirtualEnvProvider.K8S,
...
class UserHackathon(HDocumentBase):
    ...
    role = IntField(default=HackathonPlayerType.COMPETITOR)  # 0-visitor 1-admin 2-judge 3-competitor
...
class HackathonNotice(HDocumentBase):
    category = IntField()  # category: Class HackathonNotifyCategory, controls how icons/descriptions are shown at front-end
    event = IntField()  # event: Class HackathonNotifyEvent, records the specfic event that triggers current notice
    content = StringField()
...
class VirtualEnvironment(DynamicEmbeddedDocument):
    ...
    """
    provider = IntField()  # VirtualEnvProvider in enum.py
    name = StringField(required=True, unique=True)
    status = IntField(required=True)  # ExprEnvStatus in enum.py
...
class Experiment(HDocumentBase):
    status = IntField()  # ExprStatus in enum.py
