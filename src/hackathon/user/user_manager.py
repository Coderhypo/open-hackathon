...
from hackathon.constants import HttpHeader, HackathonPlayerType, FileType
from hackathon import Component, Context, RequiredFeature
from hackathon.db.models import UserToken, User, UserEmail, UserProfile, UserHackathon
...
class UserManager(Component):
    ...
    def validate_token(self):
        ...
        """
        if HttpHeader.AUTHORIZATION not in request.headers:
            return False

        user = self.__validate_token(request.headers[HttpHeader.AUTHORIZATION])
        if user is None:
            return False
    ...
    def get_user_fezzy_search(self, hackathon, args):
        ...
        def get_user_details(user):
            ...
            user_info["role"] = user_hackathon.role if user_hackathon else HackathonPlayerType.VISITOR
    ...
    def __generate_file_name(self, user_id, type, suffix):
        # may generate differrnt file_names for different type. see FileType.
