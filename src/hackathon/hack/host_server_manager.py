...
from hackathon.db.models import DockerHostServer, Hackathon
from hackathon.constants import (DockerPingResult, AVMStatus,
                                 DockerHostServerStatus, DockerHostServerState,
...
class DockerHostManager(Component):
    ...
    def get_available_docker_host(self, hackathon):
        ...
        if self.util.is_local():
            if len(vms) > 0:
                return Context(state=DockerHostServerState.SUCCESS, docker_host_server=vms[0])
            else:
                return Context(state=DockerHostServerState.FAILED)
        ...
        for host in vms:
            ...
            if not self.is_host_server_locked(host):
                return Context(state=DockerHostServerState.SUCCESS, docker_host_server=host)
            else:
                has_locked_host = True

        if has_locked_host:
            # still has available host but locked
            return Context(state=DockerHostServerState.ONGOING)
