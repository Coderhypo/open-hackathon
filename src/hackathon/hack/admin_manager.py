...
from hackathon.db.models import Hackathon, User, UserHackathon
from hackathon.constants import HackathonPlayerType, HackathonPlayerStatus
...
class AdminManager(Component):
    ...
    def validate_admin_privilege_http(self):
        ...
        return UserHackathon.objects(role=HackathonPlayerType.ADMIN, hackathon=g.hackathon, user=g.user).count() > 0
    ...
    def get_entitled_hackathons_list(self, user):
        ...
        if not user.is_super:
            hack_list = UserHackathon.objects(user=user, role=HackathonPlayerType.ADMIN).distinct("hackathon")
    ...
    def get_admins_by_hackathon(self, hackathon):
        ...
        user_hackathon_rels = UserHackathon.objects(hackathon=hackathon,
                                                    role__in=[HackathonPlayerType.ADMIN, HackathonPlayerType.JUDGE]).all()
