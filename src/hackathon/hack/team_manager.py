...
from hackathon.db.models import Team, TeamMember, TeamScore, TeamWork, Hackathon, UserHackathon, to_dic
from hackathon.hackathon_response import not_found, bad_request, precondition_failed, ok, forbidden
from hackathon.constants import TeamMemberStatus, TeamShowType, HackathonPlayerType, HackathonConfig
...
class TeamManager(Component):
    ...
    def get_hackathon_team_list(self, hackathon_id, name=None, number=None):
        ...
        def get_team(team):
            ...
            teamDic["member_count"] = team.members.filter(status=TeamMemberStatus.APPROVED).count()
    ...
    def update_team(self, kwargs):
        ...
        if "dev_plan" in kwargs and kwargs["dev_plan"] and not kwargs["dev_plan"] == "" \
                and team.hackathon.config.get(HackathonConfig.DEV_PLAN_REQUIRED, False):
    ...
    def update_team_member_status(self, operator, team_id, user_id, status):
        ...
        #   this is not allowed when status == TeamMemberStatus.APPROVED
        ...
        if status == TeamMemberStatus.APPROVED:
            ...
            # NOTE:
            #   Do we also have to delete status that is not TeamMemberStatus.APPROVED?
            ...
            mem.status = TeamMemberStatus.APPROVED
    ...
    def get_team_show_list_by_user(self, user_id):
        teams = Team.objects(members__match={
            "user": user_id,
            "status": TeamMemberStatus.APPROVED}).all()
    ...
    def get_team_source_code(self, team_id):
        try:
            team = Team.objects(id=team_id, works__type=TeamShowType.SOURCE_CODE)
        except ValidationError:
            return None
        ...
        return [w for w in team.works if w.type == TeamShowType.SOURCE_CODE][0]
    ...
    def __get_valid_team_by_user(self, user_id, hackathon_id):
        ...
        return Team.objects(
            ...
            members__match={
                "user": user_id,
                "status": TeamMemberStatus.APPROVED}).first()
    ...
    def __email_notify_dev_plan_submitted(self, team):
        ...
        admins = UserHackathon.objects(hackathon=team.hackathon, role=HackathonPlayerType.ADMIN).distinct("user")
