...
from hackathon.db.models import Hackathon, UserHackathon, DockerHostServer, User, HackathonNotice, HackathonStat, \
...
from hackathon.constants import HackathonConfig, HackathonPlayerType, HackathonStatus, HackathonPlayerStatus, \
    HttpHeader, \
    FileType, HackathonType, HackathonStat, DockerHostServerStatus, HackathonNotifyCategory, HackathonNotifyEvent, \
    OrganizationType, CloudProvider
...
class HackathonManager(Component):
    ...
    def is_recycle_enabled(self, hackathon):
        key = HackathonConfig.RECYCLE_ENABLED
    ...
    def get_hackathon_list(self, args):
        ...
        if order_by == 'create_time':  # 最新发布
            ...
            hot_hackathon_stat = HackathonStat.objects(type=HackathonStat.REGISTER, count__gt=0).order_by('-count')
    ...
    def get_online_hackathons(self):
        return Hackathon.objects(status=HackathonStatus.ONLINE)

    def get_user_hackathon_list_with_detail(self, user_id):
        user_hackathon_rels = UserHackathon.objects(user=user_id, role=HackathonPlayerType.COMPETITOR).all()
    ...
    def get_recyclable_hackathon_list(self):
        ...
        hackathons = Hackathon.objects(status=HackathonStatus.ONLINE,
    ...
    def get_recycle_minutes(self, hackathon):
        key = HackathonConfig.RECYCLE_MINUTES
        minutes = self.get_basic_property(hackathon, key, 60)
        return int(minutes)

    def validate_hackathon_name(self):
        if HttpHeader.HACKATHON_NAME in request.headers:
            try:
                hackathon_name = request.headers[HttpHeader.HACKATHON_NAME]
    ...
    def create_new_hackathon(self, context):
        ...
        self.create_hackathon_notice(new_hack.id, HackathonNotifyEvent.HACK_CREATE,
                                     HackathonNotifyCategory.HACKATHON)  # hackathon create
    ...
    def apply_online_hackathon(self, hackathon):
        ...
        try:
            req = ok()
            if hackathon.status == HackathonStatus.OFFLINE or hackathon.status == HackathonStatus.DRAFT:
                hackathon.status = HackathonStatus.APPLY_ONLINE
                hackathon.save()
    ...
    def like_hackathon(self, user, hackathon):
        ...
        if not user_hackathon:
            user_hackathon = UserHackathon(hackathon=hackathon,
                                           user=user,
                                           role=HackathonPlayerType.VISITOR,
                                           status=HackathonPlayerStatus.UNAUDIT,
                                           like=True,
                                           remark="")
            user_hackathon.save()
        ...
        self.increase_hackathon_stat(hackathon, HackathonStat.LIKE, 1)
    ...
    def unlike_hackathon(self, user, hackathon):
        ...
        self.update_hackathon_stat(hackathon, HackathonStat.LIKE, like_count)
        return ok()
    ...
    def update_hackathon_stat(self, hackathon, stat_type, count):
        ...
        :param stat_type: type of stat that defined in constants.py#HackathonStat
    ...
    def create_hackathon_notice(self, hackathon_id, notice_event, notice_category, body={}):
        ...
        :type notice_event: Class HackathonNotifyEvent
        ...
        :type notice_category: Class HackathonNotifyCategory
        :param notice_category: category that the notice belongs to, used for notice filtering and notice properties display
                                ...
                                more general than notice_event, if you want to add a new category in HackathonNotifyCategory,
        ...
        ::Example:
        :create_hackathon_notice(2, HackathonNotifyEvent.xx, HackathonNotifyCategory.yy, {'content': 'zz'})
            a new notice for a hackathon with id 2 is created for the propose of HackathonNotifyEvent.xx. The notice's front-end icon
            and description is determined by HackathonNotifyCategory.yy, while its content is 'zz' and its link url is ''

        :create_hackathon_notice(-1, HackathonNotifyEvent.xx, HackathonNotifyCategory.yy)
            a new notice not belongs to any hackathon is created for the propose of HackathonNotifyEvent.xx. The notice's front-end icon
            and description is determined by HackathonNotifyCategory.yy, while its content and link url is ''
        """
        ...
        if hackathon:
            if notice_event == HackathonNotifyEvent.HACK_CREATE:
                hackathon_notice.content = "%s即将火爆来袭，敬请期待！" % (hackathon.display_name)
            # elif notice_event == HackathonNotifyEvent.HACK_EDIT and hackathon:
            #     hackathon_notice.content = u"%s更新啦，快去看看！" % (hackathon.display_name)
            elif notice_event == HackathonNotifyEvent.HACK_ONLINE:
                hackathon_notice.content = "%s开始啦，点击报名！" % (hackathon.display_name)
                hackathon_notice.link = "/site/%s" % hackathon.name
            elif notice_event == HackathonNotifyEvent.HACK_OFFLINE:
                hackathon_notice.content = "%s圆满结束，点击查看详情！" % (hackathon.display_name)
                hackathon_notice.link = "/site/%s" % hackathon.name
            elif notice_event == HackathonNotifyEvent.HACK_PLAN and body.get('receiver', None):
                user = body.get('receiver')
                old_hackathon_notice = HackathonNotice.objects(receiver=user, event=HackathonNotifyEvent.HACK_PLAN,
        ...
        if notice_event == HackathonNotifyEvent.EXPR_JOIN and body.get('user_id'):
    ...
    def get_hackathon_notice_list(self, body):
        ...
        if hackathon_name:  # list notices that belong to specfic hackathon
            ...
            online_hackathon = Hackathon.objects(status=HackathonStatus.ONLINE)
    ...
    def check_notice_and_set_read_if_necessary(self, id):
        ...
        if hackathon_notice:
            ...
            if hackathon_notice.event == HackathonNotifyEvent.HACK_PLAN:  # set is_read = True if dev_plan is complete
    ...
    def __is_pre_allocate_enabled(self, hackathon):
        ...
        if hackathon.status != HackathonStatus.ONLINE:
            return False
        if hackathon.config.get(HackathonConfig.CLOUD_PROVIDER, CloudProvider.NONE) == CloudProvider.NONE:
            return False
        return hackathon.config.get(HackathonConfig.PRE_ALLOCATE_ENABLED, False)
    ...
    def hackathon_online(self, hackathon):
        ...
        if hackathon.status == HackathonStatus.DRAFT or hackathon.status == HackathonStatus.OFFLINE or hackathon.status == HackathonStatus.APPLY_ONLINE:
            if self.util.is_local() or hackathon.config.get('cloud_provider') == CloudProvider.NONE:
                req = ok()
        ...
        if req.get('error') is None:
            ...
            hackathon.save()
            self.create_hackathon_notice(hackathon.id, HackathonNotifyEvent.HACK_ONLINE,
    ...
    def hackathon_offline(self, hackathon):
        ...
        if hackathon.status == HackathonStatus.ONLINE or hackathon.status == HackathonStatus.DRAFT or hackathon.status == HackathonStatus.APPLY_ONLINE:
            ...
            hackathon.save()
            self.create_hackathon_notice(hackathon.id, HackathonNotifyEvent.HACK_OFFLINE,
    ...
    def __get_hackathon_detail(self, hackathon, user=None):
        ...
        for stat in HackathonStat.objects(hackathon=hackathon):
            if stat.type == HackathonStat.REGISTER:
                detail["stat"]["register"] = stat.count
            elif stat.type == HackathonStat.LIKE:
        ...
        if user:
            ...
            detail["user"]["is_admin"] = user.is_super or (
                    user_hackathon and user_hackathon.role == HackathonPlayerType.ADMIN)
            ...
            if user_hackathon and user_hackathon.role == HackathonPlayerType.COMPETITOR:
    ...
    def __fill_hackathon_detail(self, hackathon, user, hackathon_stat, user_hackathon, team):
        ...
        for stat in hackathon_stat:
            if stat.type == HackathonStat.REGISTER and stat.hackathon.id == hackathon.id:
                detail["stat"]["register"] = stat.count
            elif stat.type == HackathonStat.LIKE and stat.hackathon.id == hackathon.id:
                detail["stat"]["like"] = stat.count
            ...
            if user_hackathon:
                for uh in user_hackathon:
                    if uh.hackathon.id == hackathon.id:
                        detail['user']['admin'] = detail['user']['admin'] or (uh.role == HackathonPlayerType.ADMIN)

                        if uh.like:
                            detail['like'] = uh.like

                        if uh.role == HackathonPlayerType.COMPETITOR:
    ...
    def __create_hackathon(self, creator, context):
        ...
        new_hack = Hackathon(
            ...
            status=HackathonStatus.INIT,
            creator=creator,
            type=context.get("type", HackathonType.HACKATHON),
        ...
        try:
            admin = UserHackathon(user=creator,
                                  hackathon=new_hack,
                                  role=HackathonPlayerType.ADMIN,
                                  status=HackathonPlayerStatus.AUTO_PASSED,
                                  remark='creator')
            admin.save()
        except Exception as ex:
    ...
    def __get_pre_allocate_interval(self, hackathon):
        interval = self.get_basic_property(hackathon, HackathonConfig.PRE_ALLOCATE_INTERVAL_SECONDS)
        if interval:
            return int(interval)
        else:
    ...
    def __get_hackathon_stat(self, hackathon):
        ...
        reg_list = UserHackathon.objects(hackathon=hackathon,
                                         ...
                                         deleted=False,
                                         status__in=[HackathonPlayerStatus.AUTO_PASSED,
...
def is_auto_approve(hackathon):
    ...
    value = hack_manager.get_basic_property(hackathon, HackathonConfig.AUTO_APPROVE, "1")
